...
Let $R$ be a ring and let $\coord{M,\cdot}$ be an $R$-module.  Then, a \term{submodule}\index{Submodule} of $M$ is a subset $N\subseteq M$ such that $\coord{N,\cdot}$ is an $R$-module.
\begin{rmk}
...
\end{rmk}
\begin{rmk}
The equivalence class of $v$ with respect $\cong \bpmod{N}$ is the \term{left $N$-coset}.  The set of all left $N$-cosets is denoted by $M/N\ceqq M/\sim _{\cong \bpmod{N}}=\left\{ v+N:v\in M\right\}$\index[notation]{$M/N$}.
\end{rmk}
\begin{rmk}
By changing the definition of the equivalent relation to ``\textellipsis iff $v_1-v_2\in N$'', then we obtain the corresponding definition of \term{right $N$-cosets}, given explicitly by $N+v$.  In this case, however, the binary operation in question ($+$) is commutative, and so $r+S=S+r$, that is, the left and right cosets coincide, and so we can simply say \term{coset}.  In particular, there is no need to talk about the set of right $N$-cosets, which would have been denoted $N\backslash M$.
\end{rmk}
\begin{rmk}
...
\end{equation*}
$N$ is an \term{ideal}\index{Ideal (in an $R$-module)} iff both of these operations are well-defined.  In this case, $M/N$ is the \term{quotient module}\index{Quotient module} of $M$ modulo $N$.
\begin{rmk}
...
Let $T\colon M\rightarrow N$ be a linear transformation.  Then, the \term{kernel}\index{Kernel} of $T$, $\Ker (T)$\index[notation]{$\Ker (T)$}, is defined by
\begin{equation}
\Ker (T)\ceqq \left\{ v\in M:T(v)=0\right\} .
\end{equation}
\begin{rmk}
If the term ``kernel'' is unfamiliar to you, perhaps the term \term{null-space}\index{Null-space} is.  While I prefer to try to reserve the term ``null-space'' for matrices and ``kernel'' for linear transformations, they are essentially the same thing.
\end{rmk}
\begin{rmk}
...
Let $T\colon M\rightarrow N$ be a linear transformation.  Then, the \term{coimage}\index{Coimage} of $T$, $\Coima (T)$\index[notation]{$\Coima (T)$} is defined by
\begin{equation}
\Coima (T)\ceqq M/\Ker (T).
\end{equation}
\end{dfn}
\begin{dfn}{Cokernel}{Cokernel}
Let $T\colon M\rightarrow N$ be a linear transformation.  Then, the \term{cokernel}\index{Cokernel} of $T$, $\Coker (T)$\index[notation]{$\Coker (T)$} is defined by
\begin{equation}
...
Let $R$ be a ring and let $M$ be an $R$-module.  Then, a \term{linear-combination}\index{Linear-combination} of $v_1,\ldots ,v_m\in M$ is an element in $M$ of the form $\alpha _1\cdot v_1+\cdots +\alpha _m\cdot v_m$ for $\alpha _1,\ldots ,\alpha _m\in R$.
\end{dfn}
\begin{dfn}{Linear-independence}{LinearIndependence}
Let $R$ be a ring, let $M$ be an $R$-module, and let $S\subseteq M$.  Then, $S$ is \term{linearly-independent}\index{Linearly-independent} iff for all $m\in \Z ^+$, $\alpha _1,\ldots ,\alpha _m\in R$, and $v_1,\ldots ,v_m\in S$,
\begin{equation}
...
\begin{thm}{Span}{Span}
	Let $R$ be a ring, let $M$ be an $R$-module, and let $S\subseteq M$.  Then, there is a unique submodule of $M$, the \term{span}\index{Span} of $S$, $\Span (S)$\index[notation]{$\Span (S)$}, such that
	\begin{enumerate}
	...
	\begin{rmk}
		In the context of $R$-modules, this is usually referred to as the submodule \term{generated} by $S$.
	\end{rmk}
	\begin{proof}
		We leave this as an exercise.
		\begin{exr}[breakable=false]{}{}
