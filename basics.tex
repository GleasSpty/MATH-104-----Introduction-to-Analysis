...
\begin{rmk}
There is a good chance you may have heard of the term \emph{Cantor's Diagonal Argument}\index{Cantor's Diagonal Argument}.  The argument here is a generalization of that (it's also `cleaner'), and so we don't present the Diagonal Argument itself.
\end{rmk}
\begin{proof}
...
Let $x\in \R$.
\begin{itemize}
\item There is a unique integer $\floor{x}$\index[notation]{$\floor{x}$}, the \emph{floor}\index{Floor} of $x$, such that
\begin{enumerate}
\item $\floor{x}\leq x$; and
\item if $\Z \ni m\leq x$, then $m\leq \floor{x}$.
\end{enumerate}
\item There is a unique integer $\ceil{x}$, \index[notation]{$\ceil{x}$}, the \emph{ceiling}\index{Ceiling} of $x$, such that
\begin{enumerate}
...
\subsubsection{Polynomials and a totally-ordered field which is not Archimedean}
...
Let $F$ be a field, so that $F[x]$ is a totally-ordered integral cring.  Then, there exists unique a totally-ordered field $F(x)$, the field of \emph{rational functions with coefficients in $F$}\index{Rational functions}, such that
\begin{enumerate}
...
A \emph{directed set}\index{Directed set} is a nonempty partially-ordered set $\coord{\Lambda ,\leq}$ such that, for every $x_1,x_2\in X$, there is some $x_3\in X$ with $x_1,x_2\leq x_3$.
\begin{rmk}
In words, a partially-ordered set is directed iff you can find an upper-bound of any two elements.
\end{rmk}
\begin{rmk}
This property is sometimes called \emph{upward-directed}\index{Upward-directed}.  Compare, for example, the terminology in \cref{FilterBase,UniformSpace}.
\end{rmk}
\end{dfn}
\begin{exr}{}{}
...
\end{exm}
\begin{dfn}{Net}{}
A (real-valued) \emph{net}\index{Net} is a function from a directed set $\coord{\Lambda ,\leq}$ into $\R$.
\begin{rmk}
...
\end{rmk}
\begin{rmk}
Single values of the sequence $x_{\lambda}$ are referred to as \emph{elements}\index{Element (of a net)} or \emph{terms}\index{Term (of a net)} of the net $\lambda \mapsto x_{\lambda}$.
\end{rmk}
\begin{rmk}
...
A (real-valued) \emph{sequence}\index{Sequence} is a net whose directed set is order-isomorphic (i.e.~isomorphic in $\Pre$) to $\coord{\N ,\leq}$.
\begin{rmk}
...
Let $\Lambda \ni \lambda \mapsto x_\lambda \in \R$ be a net.  Then, $\lambda \mapsto x_\lambda$ is \emph{eventually XYZ}\index{Eventually XYZ} iff there is some $\lambda _0$ such that $\{ \lambda \in \Lambda :\lambda \geq \lambda _0\} \ni \lambda \mapsto x_\lambda$ is XYZ.
\begin{rmk}
...
Let $\Lambda \ni \lambda \mapsto x_{\lambda}\in \R$ be a net.  Then, $\lambda \mapsto x_{\lambda}$ is \emph{frequently}\index{Frequently XYZ} iff for every $\lambda \in \Lambda$ there is some $\lambda '\geq \lambda$ such that $x_{\lambda '}$ is XYZ.
\begin{rmk}
...
Let $\lambda \mapsto x_\lambda$ be a net and let $x_\infty\in \R$.  Then, $x_\infty$ is the \emph{limit}\index{Limit (of a net)} of $\lambda \mapsto x_\lambda$ iff for every $\varepsilon >0$, $\lambda \mapsto x_\lambda$ is eventually contained in $B_\varepsilon (x_\infty)$.   If a net has a limit, then we say that it \emph{converges}\index{Convergence (of net (in $\R$))}.\footnote{Divergence is not the same as nonconvergence.  We will define divergence momentarily.}
\begin{rmk}
...
The definition in \eqref{eqn2.4.23} is \emph{sigma notation}\index{Sigma notation} and the definition in \eqref{eqn2.4.24} is \emph{pi notation}\index{Pi notation}.\footnote{``$\Sigma$'' is for ``sum'' and ``$\Pi$'' is for ``product''.}
\end{rmk}
\begin{proof}
...
Let $\lambda \mapsto x_\lambda$ be a net.  Then, $\lambda \mapsto x_\lambda$ \emph{diverges to $+\infty$}\index{Diverges to $\pm \infty$} iff for every $M>0$, $\lambda \mapsto x_{\lambda}$ is eventually larger than $M$.

Similarly for $-\infty$.\footnote{You should consider writing this definition out explicitly yourself.}

$\lambda \mapsto x_\lambda$ \emph{diverges}\index{Diverges} iff $\lambda \mapsto \abs{x_{\lambda}}$ diverges.
\begin{rmk}
...
\end{equation}\footnotetext{In case you're wondering what this crazy new symbol ``$!$'' means, for $n\in \N$, $n!$, the \emph{factorial}\index{Factorial} of $n$, is defined inductively by $0!\coloneqq 1$ and $(n+1)!\coloneqq (n+1)\cdot n!$.}
...
Let $\lambda \mapsto x_\lambda$ be a net.  Then, $\lambda \mapsto x_\lambda$ is \emph{Cauchy}\index{Cauchy} iff for every $\varepsilon >0$, $\lambda \mapsto x_\lambda$ is eventually contained in some $\varepsilon$-ball.
\begin{rmk}
...
\end{exr}
Then, the \emph{limit superior}\index{Limit superior} and the \emph{limit inferior}\index{Limit inferior} of $\lambda \mapsto x_{\lambda}$, $\limsup _{\lambda}x_{\lambda}$ and $\liminf _{\lambda}x_{\lambda}$ respectively, are defined by
...
\begin{rmk}
$m\in \Z ^+$ is a \emph{perfect-square}\index{Perfect-square} iff $m=n^2$ for some $n\in \Z$.  In other words, an integer is a perfect square iff it is the square of another integer.
\end{rmk}
\begin{rmk}
...
\end{rmk}
\begin{rmk}
$r$ is the \emph{radix}\index{Radix} of the number system, and it correspond to the number of distinct symbols one uses to represent a number in a `decimal expansion' (though, strictly speaking I suppose, ``decimal expansion'' really refers to the case $r=10$).  Of course, you cannot distinguish between numbers using a single symbol alone (at least certainly not all real numbers), and so we need at least $r\geq 2$.  $r=2$ corresponds to \emph{binary}\index{Binary}, $r=3$ corresponds to \emph{ternary}\index{Ternary}, $r=8$ correspond to \emph{octal}\index{Octal}, $r=10$ corresponds to \emph{decimal}\index{Decimal}, $r=16$ corresponds to \emph{hexadecimal}\index{Hexadecimal}, and those are the only names I am aware of.
\end{rmk}
\begin{rmk}
...
Let $x\colon \Lambda \rightarrow \R$ be a be a net.  Then, a \emph{subnet}\index{Subnet} of $a$ is a net $b:\Lambda '\rightarrow \R$ such that
\begin{enumerate}
...
A \emph{cofinal subnet}\index{Cofinal subnet} of a net $x\colon \Lambda \rightarrow \R$ is a subnet of the form $\restr{a}{\Lambda '}:\Lambda '\rightarrow \R$ for $\Lambda '\subseteq \Lambda$ cofinal.
\begin{exr}[breakable=false]{}{}
...
Let $S\subseteq \R ^d$ and let $x_0\in \R ^d$.  Then, $x_0$ is a \emph{limit point}\index{Limit point} of $S$ iff there exists a net $\lambda \mapsto x_\lambda \in S$ with $x_\lambda \neq x_0$ such that $\lim _\lambda x_\lambda =x_0$.
\begin{rmk}
...
Let $D\subseteq \R ^d$,\footnote{``$D$'' is for ``domain''.} let $x_0\in \R ^d$ be a limit point of $D$, let $f\colon D\rightarrow \R ^e$, and let $L\in \R ^e$.  Then, $y$ is a \emph{limit}\index{Limit (of a function)} of $f$ at $x_0$ iff for every net $\lambda \mapsto x_\lambda \in D$ such that (i)~$x_\lambda \neq x_0$ and (ii)~$\lim _\lambda x_\lambda =x_0$ we have $\lim _\lambda f(x_\lambda )=y$.
\begin{exr}{}{}
...
Let $D\subseteq \R ^d$, let $x_0\in \R ^d$ be a limit point of $D$, and let $f\colon D\rightarrow \R ^e$.  Then, $f$ is \emph{continuous}\index{Continuous (at a point)} at $x_0$ iff $\lim _{x\to x_0}f(x)=f(x_0)$.  $f$ is \emph{continuous}\index{Continuous} iff it is continuous at $x_0$ for all $x_0\in D$.
\begin{rmk}
...
\item $a^1=a$.
\end{enumerate}
Functions of this form are called \emph{exponential functions}\index{Exponential}.  $a$ is the \emph{base}\index{Base (exponential functions)} and $x$ is the \emph{exponent}\index{Exponent (exponential functions)}.
\begin{rmk}
...
Let $U\subseteq \R ^d$.  Then, $U$ is \emph{open}\index{Open (in $\R ^d$)} iff for every $x\in U$ there is some $\varepsilon >0$ such that $B_\varepsilon (x)\subseteq U$.
\begin{rmk}
...
Let $S\subseteq \R ^d$ and let $x_0\in \R ^d$.  Then, $x_0$ is an \emph{accumulation point}\index{Accumulation point} of $S$ iff for every $\varepsilon >0$, $B_\varepsilon (x_0)$ intersects $S$ at a point \emph{distinct from} $x_0$.
\begin{rmk}
...
Let $S\subseteq \R ^d$ and let $x_0\in \R ^d$.  Then, $x_0$ is an \emph{adherent point}\index{Adherent point} of $S$ iff for every $\varepsilon >0$, $B_{\varepsilon}(x_0)$ intersects $S$.
\begin{rmk}
...
Let $S\subseteq \R ^d$ and let $x_0\in \R ^d$.  Then, $x_0$ is an \emph{isolated point}\index{Isolated point} iff there is some $\varepsilon _0>0$ such that $B_{\varepsilon _0}(x_0)\cap S=\{ x_0\}$.
\begin{rmk}
...
Let $C\subseteq \R ^d$.  Then, $C$ is \emph{perfect}\index{Perfect} iff it is equal to its set of accumulation points.
\begin{rmk}
...
Let $S\subseteq \R ^d$ and let $x_0\in \R ^d$.  Then, $x_0$ is an \emph{interior point}\index{Interior point} of $S$ iff there is some $\varepsilon _0$ such that $B_{\varepsilon _0}(x_0)\subseteq S$.
\end{dfn}
...
Let $S\subseteq \R ^d$.  Then, there exists a unique set $\Cls (S)\subseteq \R ^d$\index[notation]{$\Cls (S)$}, the \emph{closure}\index{Closure} of $S$, that satisfies
\begin{enumerate}
...
\begin{prp}{Interior}{}
Let $S\subseteq \R ^d$.  Then, there exists a unique set $\Int (S)\subseteq \R ^d$\index[notation]{$\Int (S)$}, the \emph{interior}\index{Interior} of $S$, that satisfies
\begin{enumerate}
\item $\Int (S)$ is open;
...
Let $S\subseteq \R ^d$.  Then, $S$ is \emph{quasicompact}\index{Quasicompact} iff every open cover of $S$ has a finite subcover.
\begin{rmk}
...
Let $X$ be a set, let $S\subseteq X$, let $\collection{C}\subseteq 2^X$ be a collection of subsets of $X$.  Then, $\collection{C}$ has the \emph{finite-intersection property}\index{Finite-intersection property} with $S$ iff every finite subset $\{ C_1,\ldots ,C_m\} \subseteq \collection{C}$ intersects $S$:  $(C_1\cap \cdots \cap C_m)\cap S\neq \emptyset$.  For $S=X$, we simply say that $\collection{C}$ has the \emph{finite-intersection property}.\footnote{That is, in this case we omit the phrase ``with $X$''.}
\end{dfn}
\begin{prp}{}{prp3.4.58}
